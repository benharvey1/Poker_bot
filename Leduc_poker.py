...
"Implementation of Chance Sampling CFR to create a bot for Leduc poker"
...
class Leduc:
    ...
    def get_information_set(self, card, community_card, history):
        ...
        if key not in self.i_map:
            ...
            info_set = InformationSet(key, num_actions)
...
def display_results(ev, i_map):
    ...
    for _, v in filter(lambda x: trainer.get_active_player(x[0]) == 0, sorted_items):  # Player 1's turns
        print(f"{v.key}: {v.get_average_strategy()}")  # Call the method to get the average strategy

    print("\nPlayer 2 strategies:")
    for _, v in filter(lambda x: trainer.get_active_player(x[0]) == 1, sorted_items):  # Player 2's turns
        print(f"{v.key}: {v.get_average_strategy()}")  # Call the method to get the average strategy


trainer = Leduc()
trainer.train(n_iterations=500000)